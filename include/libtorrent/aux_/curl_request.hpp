...
public:
	struct error_type {
		error_code ec = errors::no_error;
		operation_t op = operation_t::unknown;
		std::string message;

		constexpr explicit operator bool() const noexcept { return ec != errors::no_error; }
	};
	...
	[[nodiscard]] span<const char> data() const;
	...
	bool bind(const std::string& device, const address& addr);
...
private:
	...
	[[nodiscard]] bool allowed_by_ip_filter(const address& address);
