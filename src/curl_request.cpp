...
static boost::optional<address> curl_addr_to_boost(const curl_sockaddr& addr)
{
	if (addr.family == AF_INET)
	{
		...
		return boost::optional<address>{boost::in_place_init, address_v4(bytes)};
	}
	else if (addr.family == AF_INET6)
	{
		...
		return boost::optional<address>{boost::in_place_init, address_v6(bytes, sin6->sin6_scope_id)};
	}
	return {};
}
...
void curl_request::set_defaults()
{
	...
	// - CURLOPT_SSL_CIPHER_LIST (use ssl library default, this is usually a subset of the "HIGH" cipher list)
	static bool need_ssl_requirements = []() {
...
{
	// Curl's CURLOPT_TIMEOUT times the total amount of time a request takes, this includes the queuing time.
...
span<const char> curl_request::data() const
{
...
bool curl_request::bind(const std::string& device, const address& input_address)
{
	...
	// - If multiple nameservers are configured it is unclear which to use and on which interface.
	...
	const bool valid_address = input_address.is_unspecified();
	...
	address addr = input_address.is_v4() ? input_address : make_address_v6(input_address.to_v6().to_bytes());
	const auto addr_str = addr.to_string();

	// note: there are different formatting requirements for CURLOPT_DNS_INTERFACE and CURLOPT_INTERFACE
...
void curl_request::set_proxy(const proxy_settings& ps, const bool verify_ssl)
{
	...
	setopt<CURLOPT_PROXYPORT>(ps.port);
...
constexpr string_view curl_easy_option_str(CURLoption option)
{
	switch (option)
	{
...
template<typename T>
void throw_setop_ex(CURLoption option, CURLcode error, T&& value)
{
...
void curl_request::setopt(T&& value)
{
	...
	throw_setop_ex(option, error, std::forward<T>(value));
}
...
void curl_request::set_private_data(void* obj)          { setopt<CURLOPT_PRIVATE>(obj); }
void curl_request::set_ipresolve(long option)           { setopt<CURLOPT_IPRESOLVE>(option); }
void curl_request::set_ssl_verify_host(bool onoff)      { setopt<CURLOPT_SSL_VERIFYHOST>(onoff); }
void curl_request::set_ssl_verify_peer(bool onoff)      { setopt<CURLOPT_SSL_VERIFYPEER>(onoff); }
void curl_request::set_pipewait(bool onoff)             { setopt<CURLOPT_PIPEWAIT>(onoff); }
void curl_request::set_debug_logging(bool onoff)        { setopt<CURLOPT_VERBOSE>(onoff); }

#if TORRENT_ABI_VERSION == 1
void curl_request::set_userpwd(const std::string& s)    { setopt<CURLOPT_USERPWD>(s); }
#endif

void curl_request::set_write_callback(curl_write_callback option)
{
